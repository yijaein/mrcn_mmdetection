...
class LiverTestDataset(torch.utils.data.Dataset):
    CLASSES = (
        "__background__ ",
    ...
    def _preprocess_annotation(self, target, aug_det):
        ...
        for ann_info in target:
            ...
            mask, bbox_points, mask_points = img_and_key_point_augmentation(aug_det, mask, bbox_points, [mask_points])
            ...
            # pts_list = mask_points
            # bbx, bby, bbw, bbh = bbox[0], bbox[1], bbox[2] - bbox[0], bbox[3] - bbox[1]
            # fig, ax = plt.subplots(1)
            # pts_list.reverse()
            # for idx, pts in enumerate(pts_list):
            #     pts = np.reshape(np.array(pts), (-1, 2))
            #     ax.add_patch(patches.Polygon(pts, True, alpha=0.5,
