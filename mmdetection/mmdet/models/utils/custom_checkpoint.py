...
def load_state_dict(module, state_dict, strict=False, logger=None, load_type='all'):
    ...
    Args:
        ...
        load_type (str): Select 'all', 'backbone'. pretrained type -> Only load 'backbone'.
    """
    unexpected_keys = []
    rename_state_dict = set()
    own_state = module.state_dict()
    for name, param in state_dict.items():
        # backbone에 대한 load 예외 처리
        if load_type == 'backbone':
            if 'backbone' not in name:
                rename_state_dict.add(name)
                continue
    ...
    if load_type == 'backbone':
        missing_keys = set(own_state.keys()) - rename_state_dict
    else:
...
def load_checkpoint(model,
    ...
    if hasattr(model, 'module'):
        load_state_dict(model.module, state_dict, strict, logger, load_type)
    else:
        load_state_dict(model, state_dict, strict, logger, load_type)
    return checkpoint
